...
const SocietyAndBuildingFeature = ({ feature }) => {
    ...
    const nameToKeyMap = Object.fromEntries(
        Object.entries(PropertyFeatures).map(([key, value]) => [value.name, key])
    );
    ...
    return (
        ...
        <div className="society-feature-container">
            {feature?.map((item, index) => {
                ...
                const currentData = PropertyFeatures[key];
                return currentData ? (
                    <p className="society-feature" key={index}>
                        <img src={currentData.img} alt={currentData.name} /> {currentData.name}
                    </p>
                ) : null;
            })}
        </div>
